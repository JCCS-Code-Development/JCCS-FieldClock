...
export default function Home() {
  ...
  // GPS ready → reverse geocode + nearby jobs
  useEffect(() => {
    if (!position) return
  ...
  // No GPS → fall back to all assigned jobs
  useEffect(() => {
  ...
  // Pre-select active job when already clocked in
  useEffect(() => {
  ...
  const handleToggle = async () => {
    ...
    if (!isClockedIn) {
      if (!selectedJobId && !manualLocation.trim()) {
        setShowManual(true)
        setError('Select a location or type the job site name below.')
        return
      }
      setLoading(true)
      try {
        ...
        setError(err?.response?.data?.error ?? 'Could not clock in. Try again.')
      } finally { setLoading(false) }
    } else {
      setLoading(true)
      try {
        const data = await dayEnd({ lat: position?.lat, lng: position?.lng })
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 w-full">
        ...
        <div className="flex flex-col items-center gap-8 py-2">
          ...
          <div className="relative flex items-center justify-center">
            {isClockedIn && (
              <span className="absolute w-56 h-56 rounded-full animate-ping bg-red-400/20" />
            )}
            <button
