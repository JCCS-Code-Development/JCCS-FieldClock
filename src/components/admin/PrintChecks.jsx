...
function FlatRateCheckPage({ fr, today, periodStart, periodEnd, checkNum }) {
  ...
  return (
    <div className="check-page" style={{
      ...
      <div style={{ position: 'absolute', top: CHECK_CON.memo.top, left: CHECK_CON.memo.left, width: CHECK_CON.memo.w, fontSize: '11pt', fontFamily: 'Calibri, "Helvetica Neue", Arial, sans-serif', color: '#000' }}>{fr.user_name}</div>
...
export default function PrintChecks({ employees, flatRatePayments = [], period, gasByUser, bonusByUser, loanDeductions, onClose }) {
  ...
  // W-2 employees: base pay check only (gas/bonus issued as separate 1099 checks).
  // 1099 employees: full estimated total in one check.
  const allPayees = [
    ...employees
      .filter(e => e.pay_type === 'w2')
      .map(e => ({ key: `u-${e.user_id}`, name: e.name, amount: e.base_gross ?? 0, user_id: e.user_id }))
      .filter(e => e.amount > 0),
    ...employees
      .filter(e => e.pay_type !== 'w2')
    ...
    <div id="print-checks-root"
      ...
      <div style={{ padding: '28px 24px', display: 'flex', flexDirection: 'column', gap: 44, alignItems: 'center' }}>
        ...
        {employees.filter(emp => selected.has(`u-${emp.user_id}`)).map((emp) => {
          const isW2    = emp.pay_type === 'w2'
          // W-2 base check: gas/bonus go to a separate 1099 check — exclude here
          const gas     = isW2 ? 0 : (gasByUser[emp.user_id]   ?? 0)
          const bonus   = isW2 ? 0 : (bonusByUser[emp.user_id] ?? 0)
